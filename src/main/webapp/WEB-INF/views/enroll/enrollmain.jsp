...
<div id="wrap">
    ...
    <tr>
        <th>이름 입력 *</th>
        <td>
            <input type="text" id="memberName" name="memberName" style="width: 230px;" required>
        </td>
    </tr>
    <tr>
        <th>아이디 *</th>
    ...
    <td>
        ...
        <!-- margin-left 값을 32px에서 더 늘려서 조정 -->
...
//회원구분 라디오 버튼 변경 시 학교/학원 선택 UI 변경
$('input[name="eduType"]').on('change', function() {
    ...
    if(selectedType === '1') {  // 학교 선택 시
        ...
        $('#school-name').html('<option value="">학원명</option>');
        // 학원 검색용 select 표시
    }
});

// 지역 선택 시 해당 구/군 목록 불러오기
...
// 구/군 선택 시 해당 학교/학원 목록 불러오기
function schoolSearch(e) {
    const select = $("#school-name");
    select.html("<option value=''>학교/학원명</option>");
    ...
    $.ajax({
        url: eduType === '1' ? `${path}/school/selectchildschool` : `${path}/school/selectacademy`,
        type: "GET",
        data: { 
            district: district,
            eduType: eduType 
